...
private Map<String, String> renewPageCookies;
    ArrayList<BookDetails> booklist=new ArrayList<>();

    public ArrayList<BookDetails> getWebsite(final MainActivity.bookCallback rCallback, final UserDetails user) {
...
//
                    Connection.Response loginForm1 = Jsoup.connect("https://webauth.dbs.ie/idp/profile/SAML2/Redirect/SSO?execution=e1s1")
                            ...
                            .timeout(10 * 1000)
                            //post parameters
                            .data("j_username",user.getDbsID())
                            .data("j_password", user.getPswd())
...
//                               .followRedirects(true)
                    ...
                    renewPageCookies=mapLoginPageCookies5;
...
//                    Elements booklist = document2.select(".author");
                    ...
                    Element nbooks = document2.select("a[href*=#opac-user-checkouts]").first();
                    Element bfine= document2.select("a[href*=#opac-user-fines]").first();

                    Element fname=document2.select("a[href*=/cgi-bin/koha/opac-user.pl]").first();
                    MainActivity.user.setBook(nbooks.text());
MainActivity.user.setName(fname.text());

if(bfine!=null){
    MainActivity.user.setFine(bfine.text());
}else
{
    MainActivity.user.setFine("Fine(0)");
}
                    System.out.println("Blog:"+title.size());
                    System.out.println("Blog:"+booklist.size());
if(title!=null) {
    booklist.clear();
...
//    rCallback.onCallback(booklist);
    ...
    public void renewBook(final String link){

        System.out.println(renewPageCookies);
        new Thread( new Runnable() {
            @Override
            public void run() {
                ...
                try {
                    ...
                    Connection.Response loginForm4 = Jsoup.connect(link)
                            //referrer will be the login page's URL
                            .referrer("https://webauth.dbs.ie/idp/profile/SAML2/Redirect/SSO?execution=e1s2")
                            //user agent
                            .userAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.142 Safari/537.36")
